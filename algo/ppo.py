    ...
    def update(self, rollouts):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                returns = sample['returns']
                adv_targ = sample['adv_targ']
                rec_states = sample['rec_states']   # Recurrent states

                # Reshape to do in a single forward pass for all steps
                values, action_log_probs, dist_entropy, _ = \
                    self.actor_critic.evaluate_actions(
                        sample['state'], sample['goal_cat_ids'], (rec_states[:, 0].transpose(0, 1), rec_states[:, 1].transpose(0, 1)),
                        sample['actions'], sample['detection_results'], sample['gat_embedding_memory']
                    )
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                values, action_log_probs, dist_entropy, _ = \
                    self.actor_critic.evaluate_actions(
                        sample['obs'], sample['rec_states'],
                        sample['masks'], sample['actions'], sample['detection_results'],        # for sog w.o. gat_memory
                        # sample['masks'], sample['actions'], sample['detection_results'], sample['gat_embedding_memory'], 
                        extras=sample['extras']
                    )
