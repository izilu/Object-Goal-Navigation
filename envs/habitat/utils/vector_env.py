...
class TSOGVectorEnv:
            ...
            while command != CLOSE_COMMAND:
                if command == STEP_COMMAND:
                    ...
                    if isinstance(env, habitat.RLEnv) or isinstance(
                        ...
                        # habitat.RLEnv
                        state, detection_result, reward, done, info = env.step(**data)
                        if auto_reset_done and done:
                            state, detection_result, info = env.reset()
                        connection_write_fn((state, detection_result, reward, done, info))
                    elif isinstance(env, habitat.Env):
                        # habitat.Env
                        observations = env.step(**data)
                        if auto_reset_done and env.episode_over:
                ...
                # TODO: update CALL_COMMAND for getting attribute like this
                    ...
                    state, detection_result, reward, done, info = \
                            env.plan_act_and_preprocess(data)
                    if auto_reset_done and done:
                        state, detection_result, info = env.reset()
                    connection_write_fn((state, detection_result, reward, done, info))
    ...
    def reset(self):
        ...
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        state, dectection_results, infos = zip(*results)

        self._is_waiting = False
        return np.stack(state), np.stack(dectection_results), infos
    ...
    def step_wait(self) -> List[Observations]:
        ...
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        self._is_waiting = False
        state, detection_result, rews, dones, infos = zip(*results)
        return np.stack(state), np.stack(detection_result), np.stack(rews), np.stack(dones), infos
